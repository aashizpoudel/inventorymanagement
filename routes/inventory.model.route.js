...
module.exports = function(router) {
		...
		.put(function(req, res) {
			...
			Item.findOne({itemNumber: req.body.itemNumber}, function(err, item) {
				if (err) {
					...
					item.quantity = inventory.quantity + req.body.quantity;
					item.uom = req.body.uom;
					item.children = req.body.children;					
					...
					item.save(function(err) {
						if (err) {
							...
							res.json({message: req.body.itemNumber + ' modified' });
						}
	...
	router.route('/add/addNewItem/')
		/*
			...
			A new item is added to inventory to inventory.
			The HTTP request contains the following values:
                itemNumber 	- Item ID string
                itemName - Item Name String
				desc		- Item description
				quantity	- Item quantity on file
				uom			- Item unit of measurement
		...
		*/
		.post(function(req, res) {
            ...
            inventory.itemNumber = req.body.itemNumber;
